...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    var loc = req.url.slice(1).split('/m3u8/')[1] ? req.url.slice(1).split('/m3u8/')[0] : location ;
    switch(loc){
      case 'mlb' :
        location = parseURL('http://mlb-nhl.streams.rip/mlb/m3u8/'+req.url.slice(1).split('/m3u8/')[1]);
        break;
      case 'nhl' :
        location = parseURL('http://mlb-nhl.streams.rip/m3u8/'+req.url.slice(1).split('/m3u8/')[1]);
        break;
      default:
        location = parseURL('http://mlb-nhl.streams.rip/'+req.url.slice(1));
        break;
                   }
    ...
    if (corsAnywhere.originBlacklist.indexOf(origin) >= 0) {
      ...
      res.end('The origin "' + origin + '" was blacklisted by streams.rip');
      return;
    }

    if (corsAnywhere.originWhitelist.length && corsAnywhere.originWhitelist.indexOf(origin) === -1) {
      res.writeHead(403, 'Forbidden', cors_headers);
      res.end('The origin "' + origin + '" was not whitelisted by streams.rip');
      return;
    }
